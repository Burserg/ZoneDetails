local debug = false

local L = LibStub("AceLocale-3.0"):NewLocale("ZoneDetails", "enUS", true, debug)

if not L then return end

L["Greetings Message"] = true
L["Display settings for Hearth greeting"] = true
L["Message"] = true
L["The Message to be displayed when you enter the area where your Hearthstone is set."] = true
L["Show Message"] = true
L["Toggles the display of greeting message."] = true
L["Map Settings"] = true
L["Items displayed on the map"] = true
L["Show Instance Text"] = true
L["Toggles the display of instances that can be found in current zone."] = true
L["Show Instance Entrance"] = true
L["Toggles the display of instance entrance."] = true
L["Show Raid Text"] = true
L["Toggles the display of raids."] = true
L["Show Raid Entrance"] = true
L["Toggles the display of raid entrance."] = true
L["Show Battlegrounds"] = true
L["Toggles the display of battlegrounds."] = true
L["Show Continent Hover"] = true
L["Toggles showing a zone's details in the corner overlay when you hover it on a continent map."] = true
L["Profession Settings"] = true
L["Profession details displayed on the map"] = true
L["Professions"] = true
L["Show Fishing"] = true
L["Toggles the display of Fishing Skill on the map."] = true
L["Show Herbs"] = true
L["Toggles the display of herbs that can be found in current zone."] = true
L["Show Minerals"] = true
L["Toggles the display of minerals that can be found in current zone."] = true
L["Text Size"] = true
L["Zone Text Size"] = true
L["Adjusts the size of the zone name text shown on the map."] = true
L["Instance Text Size"] = true
L["Adjusts the size of the static instance, raid, and battleground block shown on the map."] = true
L["Hover Text Size"] = true
L["Adjusts the size of the instance/raid details shown above the cursor when hovering their map icons."] = true
L["Profession Text Size"] = true
L["Adjusts the size of the profession text shown on the map."] = true
L["Instances"] = true
L["Battlegrounds"] = true
L["Raids"] = true
L["Mining"] = true
L["Herbalism"] = true
L["Fishing"] = true
L["Fishing Minimum"] = true
L["Herbs"] = true
L["Nodes"] = true

L["Scarlet Monastery: Graveyard"] = true
L["Scarlet Monastery: Library"] = true
L["Scarlet Monastery: Armory"] = true
L["Scarlet Monastery: Cathedral"] = true
L["Dire Maul: East"] = true
L["Dire Maul: West"] = true
L["Dire Maul: North"] = true

L["Demon Fall Canyon"] = true
L["Scarlet Enclave"] = true
L["Upper Blackrock Spire"] = true
L["Karazhan Crypts"] = true
L["Old Hillsbrad Foothills"] = true
L["The Black Morass"] = true
L["Hyjal Summit"] = true
L["Player"] = true

L["Map Enhancements"] = true
L["Fog of war reveal and a resizable windowed map"] = true
L["Fog of War"] = true
L["Reveal Unexplored Areas"] = true
L["Shows unexplored areas of the map using the game's own map art."] = true
L["Tint Unexplored Areas"] = true
L["Applies a tint to unexplored areas so they stand out from explored ones."] = true
L["Tint Color"] = true
L["The color and opacity applied to unexplored areas."] = true
L["Map Window"] = true
L["Replace Full-Screen Map"] = true
L["Keeps the World Map windowed instead of full-screen. Drag the map border to move it and drag the grip in the bottom-right corner to resize it."] = true
L["Map Scale"] = true
L["Adjusts the size of the windowed map."] = true
